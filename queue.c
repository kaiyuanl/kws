...
int kws_queue_init(struct kws_queue *queue, unsigned int size)
{
	...
	memset(items, NULL, size * sizeof(void *));
...
int kws_request_queue_init(struct kws_queue *queue)
{
	return kws_queue_init(queue);
}
...
int kws_request_queue_in(struct kws_queue *queue,
